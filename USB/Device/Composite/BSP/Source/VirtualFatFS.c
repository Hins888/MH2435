...
static void     write_fat_tbl(uint32_t offset, const uint8_t* data, uint32_t size);
...
static const mbr_t mbr_tmpl = {
    ...
    /*char[11]*/ .volume_label = {'V', 'I', 'R', 'T', 'U', 'A', 'L', '-', 'F', 'A', 'T'},
    ...
    32 0000002E C3                              ret
    33
    34 0000002F 504C45415345205245-     message db 'PLEASE REMOVE THE USB DEVICE AND REBOOT THE SYSTEM..', 0
    35 00000038 4D4F56452054484520-
    36 00000041 555342204445564943-
    37 0000004A 4520414E4420524542-
    38 00000053 4F4F54205448452053-
    39 0000005C 595354454D2E2E00
    42
    ...
    /*uint8_t[448]*/ .bootstrap =
        {
            ...
            0x20, 0x55, 0x53, 0x42, 0x20, 0x44, 0x45, 0x56, 0x49, 0x43, 0x45, 0x20, 0x41, 0x4E, 0x44, 0x20, //
            0x52, 0x45, 0x42, 0x4F, 0x4F, 0x54, 0x20, 0x54, 0x48, 0x45, 0x20, 0x53, 0x59, 0x53, 0x54, 0x45, //
            0x4D, 0x2E, 0x2E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //
...
const virtual_media_t virtual_media_tmpl[] = {
    ...
    {read_fat, write_fat_tbl, 0 /* Set at runtime */}, /* FAT1         */
...
uint32_t vfs_read(uint32_t requested_sector, uint8_t* buf, uint32_t num_sectors) {
    uint8_t  i = 0;
    ...
    for (i = 0; i < ARRAY_SIZE(virtual_media); i++) {
        ...
        if ((requested_sector >= vm_start) && (requested_sector < vm_end)) {
            ...
            if (virtual_media[i].read_cb(sector_offset, buf, sectors_to_write) != sectors_to_write * VFS_SECTOR_SIZE)
                return 0;
...
static uint32_t read_fat(uint32_t sector_offset, uint8_t* data, uint32_t num_sectors) {
    uint32_t read_size = num_sectors * VFS_SECTOR_SIZE;
    //COMPILER_ASSERT(sizeof(file_allocation_table_t) <= VFS_SECTOR_SIZE);

    if (sector_offset >= sizeof(file_allocation_table_t) / VFS_SECTOR_SIZE) {
        // Don't worry about reading other sectors
        return VFS_SECTOR_SIZE;
    }

    memcpy(data, &fat.f[sector_offset * VFS_SECTOR_SIZE], read_size);
    return VFS_SECTOR_SIZE;
}

static void write_fat_tbl(uint32_t sector_offset, const uint8_t* data, uint32_t num_sectors) {
    uint32_t write_size = num_sectors * VFS_SECTOR_SIZE;
    //COMPILER_ASSERT(sizeof(file_allocation_table_t) <= VFS_SECTOR_SIZE);
    ...
    memcpy(&fat.f[sector_offset * VFS_SECTOR_SIZE], data, write_size);
    return;
}
