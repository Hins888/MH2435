...
#ifndef CONFIG_USBCDC_SERVICE_SEND_BYTE_BUFFER_SIZE
#define CONFIG_USBCDC_SERVICE_SEND_BYTE_BUFFER_SIZE (4096*4)
#endif

#ifndef CONFIG_USBCDC_SERVICE_READ_BYTE_BUFFER_SIZE
#define CONFIG_USBCDC_SERVICE_READ_BYTE_BUFFER_SIZE (4096*4)
#endif
...
typedef struct CDC_UserCb
{
  ...
  bool (*ReadCallback)(uint32_t* buffer, uint32_t bytesRead);
} CDC_Usercb_TypeDef;
...
extern uint32_t VCPHostSendBytes(uint8_t* buffer, uint32_t size);
extern uint32_t VCPHostSendBufferSize(bool isFreeSize);

extern uint32_t VCPHostReadBytes(uint8_t* buffer, uint32_t size);
extern uint32_t VCPHostReadBufferSize(bool isFreeSize);
