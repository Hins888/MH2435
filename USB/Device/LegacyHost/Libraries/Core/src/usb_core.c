...
USB_OTG_STS USB_OTG_HC_Init(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  // Meano: temporary enable nak interrupt for non-DMA mode
  if (pdev->cfg.dma_enable == 0 &&  pdev->host.hc[hc_num].ep_type != EP_TYPE_ISOC)
  {
    hcintmsk.b.nak = 1;
  }
...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  {
      pdev->host.hc[hc_num].out_num_packets  = num_packets;
  }
