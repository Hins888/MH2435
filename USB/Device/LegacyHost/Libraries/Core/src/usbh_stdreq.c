...

{
  ...
  if((status = USBH_GetDescriptor(pdev,
                                  phost,
                                  USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD,                          
                                  USB_DESC_CONFIGURATION | USER_SELECTED_DESC_CONFIGURATION_NUM, 
                                  pdev->host.Rx_Buffer,
                                  length)) == USBH_OK)
  {
...
USBH_Status USBH_SetInterface(USB_OTG_CORE_HANDLE *pdev, 
                        USBH_HOST *phost,
                        uint8_t itf_idx, uint8_t altSetting)
{
  ...
  phost->Control.setup.b.wIndex.w = itf_idx;
  phost->Control.setup.b.wLength.w = 0;           
...
{  
  ...
  if (length > USB_CONFIGURATION_DESC_SIZE)
  {
    ...
    while ((if_ix < USBH_MAX_NUM_INTERFACES ) && (ptr < cfg_desc->wTotalLength))
    {
      ...
      {
        pif = &itf_desc[if_ix];//&cfg_desc->itf_desc[if_ix];
        USBH_ParseInterfaceDesc (pif, (uint8_t *)pdesc);            
        
        ep_ix = 0;
        pep = (USBH_EpDesc_TypeDef *)0;        
