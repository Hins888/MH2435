...
extern struct netif *p_eth_netif;
...
int set_ipaddr(struct netif *p_netif, struct ip_addr *ipaddr, struct ip_addr *netmask, struct ip_addr *gw, struct ip_addr *dns)
{
    ...
    netif_set_down(p_eth_netif);
    netif_set_ipaddr(p_eth_netif, ipaddr);
    netif_set_netmask(p_eth_netif, netmask);
    netif_set_gw(p_eth_netif, gw);
    dns_setserver(0, dns);
...
int udp_add_membership(char *ip, uint16_t port)
{
    ...
    imr.imr_interface.s_addr =  p_eth_netif->ip_addr.addr;
