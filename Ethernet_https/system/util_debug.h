...
#ifdef DEBUG_TASK
#define p_dbg_enter_task  do{if(!dbg_level)break;printf("[D: %d.%03d] ",  os_time_get()/1000, os_time_get()%1000);printf("enter %s 任务\n", __FUNCTION__); printf("\r\n");}while(0)   //进入任务的函数
#define p_dbg_exit_task  do{if(!dbg_level)break;printf("[D: %d.%03d] ",  os_time_get()/1000, os_time_get()%1000);printf("exit %s 任务\n", __FUNCTION__); printf("\r\n");}while(0)     //退出任务的函数
#else
#define p_dbg_enter_task 
#define p_dbg_exit_task 
#endif
#ifndef  assert
#define assert(x)                                                       \
do{                                                                     \
