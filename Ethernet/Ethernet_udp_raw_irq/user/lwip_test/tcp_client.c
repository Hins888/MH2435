...
{   
    ...
    switch(g_tcp_step)
    {
        case 0:
            ...
            printf("connected service: %d.%d.%d.%d port:%d\n", TCP_SERVICE_IP1, 
                    TCP_SERVICE_IP2, TCP_SERVICE_IP3, TCP_SERVICE_IP4, SERVER_TCP_PORT);
            g_tcp_step = 1;
            break;
        case 1:
            //lwip_test_buf =
...
err_t tcp_client_connected(void *arg)
{
    ...
    if (tcppcb)           /* 创建成功 */
    {
        ...
        //printf("Remote IP:%d.%d.%d.%d:", remoteip[0], remoteip[1], remoteip[2], remoteip[3]); /* Ô¶¶ËIP */
