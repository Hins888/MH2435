...
void ETH_MACDMA_DeInit()
{
    ETH_DeInit();
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_ETH | RCC_AHB1Periph_ETH_TX | RCC_AHB1Periph_ETH_RX, DISABLE);
}
...
static void ETH_MACDMA_Config(void)
{
    ...
    /* Configure Ethernet */
    EthStatus = ETH_Init(&ETH_InitStructure, &ETH_PhyDescStructure);
...
void ETH_GPIO_Config(void)
{
    ...
    /* Enable GPIOs clocks */
    RCC_AHB1PeriphClockCmd(ETH_MDIO_GPIO_CLK | ETH_MDC_GPIO_CLK | ETH_RMII_REF_CLK_GPIO_CLK | ETH_RMII_CRS_DV_GPIO_CLK | ETH_RMII_RXD0_GPIO_CLK |
                           ETH_RMII_RXD1_GPIO_CLK | ETH_RMII_TX_EN_GPIO_CLK | ETH_RMII_TXD0_GPIO_CLK | ETH_RMII_TXD1_GPIO_CLK | ETH_PHY_RST_GPIO_CLK,
                           ENABLE);
    ...
    ETH_MediaInterfaceConfig(SYSCFG_ETH_MediaInterface_MII);
#elif defined RMII_MODE /* Mode RMII*/

    ETH_MediaInterfaceConfig(SYSCFG_ETH_MediaInterface_RMII);
#endif
    ...
    /* MAC PHY RST */	
	GPIO_InitStructure.GPIO_Pin = ETH_PHY_RST_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_Init(ETH_PHY_RST_PORT, &GPIO_InitStructure);	
...
void ETH_link_callback(struct netif* netif)
{
    ...
    if (netif_is_link_up(netif))
    {
        ...
        if (ETH_InitStructure.ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
        {
            ...
            RegValue = ETH_ReadPHYRegister(ETHERNET_PHY_ADDRESS, PHY_RTL8201F_SR);

            /* Configure the MAC with the Duplex Mode fixed by the auto-negotiation process */
            if ((RegValue & PHY_RTL8201F_DUPLEX_STATUS) != (uint32_t)RESET)
            {
            ...
            if (RegValue & PHY_RTL8201F_SPEED_STATUS)
            {
...
void ETH_IRQHandler(void)
{
    ...
    /* »ñÈ¡ÐÅºÅÁ¿ */
    xSemaphoreGiveFromISR(s_xSemaphore,&xHigherPriorityTaskWoken);/* ÊÍ·Å¶þÖµÐÅºÅÁ¿ */
