...
************************************************************/
static int32_t errno_transform(int32_t systemErrno, NETWORK_ERROR_E *netwokErrno, int32_t *privateErrno)
{
...
* 说       明: 将linux系统下的errno转换为sdk自定义的errno
************************************************************/
int32_t socket_get_errno(void)
{
    NETWORK_ERROR_E networkErrno = NETWORK_FAIL;
    int32_t private = 0;
    int32_t result = errno_transform(errno, &networkErrno, &private);
    if(0 != result)
    {
        NET_LOG_ERROR("network errno = %d",errno);
        return NETWORK_FAIL;
    }
...
/***********************************************************
* 函数名称: network_socket_send
* 描       述: 发送网络数据接口
* 输入参数: int32_t sockFd socket描述符
*           void *buf 数据缓存
*           int32_t nbytes 缓存区大小
*           SOCKET_TRANS_FLAGS_E flags 操作标志
* 输出参数:
* 返 回  值: 同linux系统下标准socket返回值
* 说       明: 网络数据发送，linux系统下的实现
*           源码和mbedtls中使用
************************************************************/
int32_t network_socket_send(int32_t sockFd, void *buf, int32_t nbytes, uint32_t flags)
{
    uint32_t flag = 0;
...
*           源码和mbedtls中使用
************************************************************/
int32_t network_socket_recv(int32_t sockFd, void *buf, int32_t nbytes, uint32_t flags)
{
    uint32_t flag = 0;
...
/***********************************************************
* 函数名称: network_socket_select
* 描       述: 有可读可写事件判断接口
* 输入参数: int32_t fd socket描述符
*           SOCKET_TRANS_TYPE_E type 判断读写事件类型
*           int timeoutMs 超时时间
* 输出参数: SOCKET_FD_ISSET_E* result 可读可写事件结果
* 返 回  值: 同linux系统下标准select的返回值
* 说       明: 判断socket是否有可读写事件，使用linux系统下select接口进行实现
*           源码和mbedtls中使用
************************************************************/
int32_t network_socket_select(int32_t fd, SOCKET_TRANS_TYPE_E type, int timeoutMs, SOCKET_FD_ISSET_E* result)
{
...
/***********************************************************
* 函数名称: network_socket_settimeout
* 描       述: 设置socketopt的超时参数
* 输入参数: int32_t fd socket描述符
*           int timeoutMs 超时时间
*           SOCKET_TRANS_TYPE_E type 读写类型
* 输出参数:
* 返 回  值: 0：成功 非零：失败
* 说       明: linux系统下通过setsocketopt接口设置收发数据的超时时间
*           源码和mbedtls中使用
************************************************************/
int32_t network_socket_settimeout(int32_t fd,int timeoutMs,SOCKET_TRANS_TYPE_E type)
{
    struct timeval timeout ;//= {timeoutMs/1000, (timeoutMs%1000)*1000};

    int optname = type == SOCKET_TRANS_RECV ? SO_RCVTIMEO:SO_SNDTIMEO;
    timeout.tv_sec = timeoutMs/1000;
    timeout.tv_usec = (timeoutMs%1000)*1000;
    if( fd < 0 )
    {
    ...
    {
        NET_LOG_ERROR("setsockopt error, errno = %d",errno);
        return NET_ESETOPT_TIMEOUT;
    }
...
/***********************************************************
* 函数名称: network_socket_get_nonblock
* 描       述: 获取socket非阻塞状态
* 输入参数: int32_t fd socket描述符
* 输出参数:
* 返 回  值: 0：阻塞  非零：非阻塞
...
*           mbedtls中使用
************************************************************/
int32_t network_socket_get_nonblock(int32_t fd)
{
    if( fd < 0 )
    {
        return NETWORK_FAIL;
...
/***********************************************************
* 函数名称: network_socket_set_nonblock
* 描       述: 设置socket非阻塞状态
* 输入参数: int32_t fd socket描述符
* 输出参数:
* 返 回  值: 0：成功  非零：失败
...
*           mbedtls中使用
************************************************************/
int32_t network_socket_set_nonblock(int32_t fd)
{
    int32_t flags = 0;
    if( fd < 0 )
    {
...
*           mbedtls中使用
************************************************************/
int32_t network_socket_set_block(int32_t fd)
{
    int32_t flags = 0;
    if( fd < 0 )
    {
...
*           mbedtls中使用
************************************************************/
int32_t network_socket_close(int32_t fd)
{
    return close(fd);
}

/***********************************************************
* 函数名称: network_socket_shutdown
...
*           mbedtls中使用
************************************************************/
int32_t network_socket_shutdown(int32_t fd,int32_t how)
{
    return shutdown(fd,how);
}

/***********************************************************
* 函数名称: network_socket_create
* 描       述: 客户端网络资源创建、连接
* 输入参数: int8_t*host 域名或主机IP
*           int8_t*service 端口或服务名
*           SOCKET_PROTOCOL_TYPE type 协议类型
* 输出参数:
* 返 回  值:  0：成功，非零：失败
...
*           源码以及mbedtls中使用
************************************************************/
int32_t network_socket_create(const int8_t*host,const int8_t*service,SOCKET_PROTOCOL_TYPE type)
{
    struct addrinfo hints;
    struct addrinfo *addrInfoList = NULL;
    struct addrinfo *cur = NULL;
    int fd = 0;
    int rc = NET_EUNKNOWN_HOST;

    memset( &hints, 0, sizeof(hints));

    //默认支持IPv4的服务
    if(SOCKET_PROTOCOL_TCP == type)
    {
    ...
    if ((rc = getaddrinfo((const char *)host, (const char *)service, &hints, &addrInfoList ))!= 0 )
    {
        NET_LOG_ERROR("getaddrinfo error! rc = %d, errno = %d",rc,errno);
        return NET_EUNKNOWN_HOST;
    }
    ...
    for( cur = addrInfoList; cur != NULL; cur = cur->ai_next )
    {
        ...
        {
            NET_LOG_ERROR("socket type error");
            rc = NET_ESOCKET;
            continue;
        }
        ...
        if( fd < 0 )
        {
            NET_LOG_ERROR("create socket error,fd = %d, errno = %d",fd,errno);
            rc = NET_ESOCKET;
            continue;
        }
        ...
        close( fd );
        NET_LOG_ERROR("connect error,errno = %d",errno);
        rc = NET_ECONNECT;
    }
...
*           mbedtls中使用
************************************************************/
int32_t network_socket_bind(const int8_t*host,const int8_t*service, SOCKET_PROTOCOL_TYPE type)
{
    int fd = 0;
    int n = 0;
    int ret = FAIL_RETURN;
    ...
    memset( &hints, 0, sizeof( hints ) );
    hints.ai_family = AF_INET;
    hints.ai_socktype = type == SOCKET_PROTOCOL_UDP ? SOCK_DGRAM : SOCK_STREAM;
    hints.ai_protocol = type == SOCKET_PROTOCOL_UDP ? IPPROTO_UDP : IPPROTO_TCP;
    if( host == NULL )
    {
        hints.ai_flags = 0x01;
    }
